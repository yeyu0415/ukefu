...
import com.ukefu.webim.service.impl.ContactsDataExchangeImpl;
import com.ukefu.webim.service.impl.OrganDataExchangeImpl;
...
public class ContactsController extends Handler{
	...
	@Autowired
	private ContactsDataExchangeImpl contactsDataExchangeImpl;
	...
	@GetMapping(value = "/embed/syncontacts")
	@Menu(type = "contacts" , subtype = "embedsave")
	@ResponseBody
	public Iterator synContactsData() {
    	int pageNum=1;
    	int pageSize=1000;

		Iterator<Contacts> list= contactsDataExchangeImpl.queryList(pageNum,pageSize);
		while(list!=null) {
			while (list.hasNext()) {
				Contacts c=list.next();
				contactsRes.save(c);
			}
			pageNum++;
			list= contactsDataExchangeImpl.queryList(pageNum,pageSize);
		}
		return list;
	}
